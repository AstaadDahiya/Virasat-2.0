    ...
    <div className="flex min-h-screen flex-col">
      ...
      <main className="flex-1 bg-secondary">
        <div className="container mx-auto px-4 py-12 md:py-20">
            ...
            <CardContent className="p-6 md:p-8">
              ...
              <div className="grid md:grid-cols-3 gap-8 mt-8">
                <div className="md:col-span-2">
                  <h2 className="font-headline text-2xl font-bold mb-4">{t('nav.about')} {artisanName}</h2>
                  <p className="text-muted-foreground leading-relaxed whitespace-pre-wrap">{language === 'hi' ? artisan.bio_hi : artisan.bio}</p>
                </div>
                <div>
                   <h2 className="font-headline text-2xl font-bold mb-4">{t('nav.contact')} {artisanName}</h2>
                   <ContactArtisanForm />
                </div>
              </div>
            </CardContent>
          ...
          <div className="mt-16">
            <h2 className="font-headline text-3xl font-bold mb-6">{t('product.productsBy')} {artisanName}</h2>
            {products.length > 0 ? (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-8">
               ...
               <p className="text-muted-foreground">{t('product.noProductsListed')}</p>
            )}
          </div>
        </div>
      </main>
